...
-- 제미나이
-- 1. 먼저 고객별로 7월 총 매출을 계산하는 임시 테이블(CTE)을 만듭니다.
WITH july_sales AS (
    SELECT
        c.customer_name,
        SUM(o.amount) AS total_july_sales
    FROM customers c
    JOIN orders o ON c.customer_id = o.customer_id
    WHERE o.order_date BETWEEN '2024-07-01' AND '2024-07-31'
    GROUP BY c.customer_id, c.customer_name -- 고객 ID와 이름으로 그룹화
)
-- 2. 위에서 만든 임시 테이블을 사용해 순위를 매기고 최종 결과를 선택합니다.
SELECT
    customer_name AS "이름",
    total_july_sales AS "7월 매출",
    RANK() OVER (ORDER BY total_july_sales DESC) AS "순위"
FROM july_sales
ORDER BY "순위"
LIMIT 3;
