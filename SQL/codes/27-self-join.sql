...
-- 고객 간 구매 패턴 유사성
-- customers <-ij-> sales <-ij-> customers    *ij = inner join
...
-- 1번 손님의 구매 데이터
FROM customers c1                                           -- 1번 손님의 모두 다
INNER JOIN sales s1 ON c1.customer_id = s1.customer_id      -- 1번 손님의 구매 데이터
-- 2번 손님의 구매 데이터
INNER JOIN customers c2 ON c1.customer_id < c2.customer_id  -- 1번 손님과 다른 사람(2번)을 고르는 중 (중복 데이터 제거)
